...
import { setNodesInstances } from "../../features/graph/graphSlice";
import Nearby from "nearby-js/build/Nearby-es";
...
import { useDocumentStore, useGraphStore } from "../../stores/store";
...
function saveControllerState(
  ...
  if (grabbingController.current) {
    grabbingController.current.getWorldPosition(tempVector);
    positionBuffer.current.push(tempVector.clone());
...
>(
  (
    ...
    function checkNode(handedness, nodeId) {
      ...
      for (let object of result.keys()) {
        ...
        if (distance < 10 && object.id.startsWith("n")) {
          ...
          setSelectedNodeIds([+nodeId, +toBeLinked.current]);
          setSelectedText({ text: " ", from: -1 }); // HACK: to trigger the preview
          found = true;
        }
      }
      ...
      if (!found) {
        ...
        setSelectedNodeIds([]);
        setSelectedText({ text: "", from: -1 }); // HACK: to hide the preview
      }
    ...
    useFrame(() => {
      ...
      const clickedNodes = useGraphStore
        .getState()
        .selectedNodeIds.map((id) => {
          return getNodeIndex(`${id}`, idIndexMap);
        });

      if (rightController && config.isCollaborative) {
        // voronoi
      ...
      if (instance) {
        ...
        otherHighlightedNodes.forEach((nodeStatus, i) => {
          const targetNodeId = getNodeIndex(`${nodeStatus.id}`, idIndexMap);
      ...
      links.forEach((link, i) => {
        ...
        let source = getNodeIndex(`${link.source}`, idIndexMap);
        const target = getNodeIndex(`${link.target}`, idIndexMap);
      ...
      saveControllerState(rightGrabbingController, rightPositionBuffer, temp2);
      saveControllerState(leftGrabbingController, leftPositionBuffer, temp2);

      if (rightPickedObjects.length > 0 && leftPickedObjects.length > 0) {
        console.log(rightPickedObjects, leftPickedObjects);
    ...
    const onSqeezeStart = useCallback(
      (e) => {
        ...
        const previousTransform =
          handedness === "right"
            ? rightPreviousTransform
            : leftPreviousTransform;
        const grabbingController =
          handedness === "right"
            ? rightGrabbingController
            : leftGrabbingController;
        ...
        console.log("picked objects: ", res, handedness);

        setPickedObjects(res);
      },
      [rightPickedObjects, leftPickedObjects]
    );

    const onSqeezeEnd = useCallback(
      (e) => {
        ...
        console.log("pickedObjects: ", pickedObjects, handedness);

        pickedObjects.forEach((oid) => {
          // node
          if (oid.startsWith("n")) {
            let nodeId = +oid.slice(1);
            let id = getNodeIndex(`${nodeId}`, idIndexMap);
            ...
            if (Math.abs(velocities[velocities.length - 1].length()) > 0.03) {
              ...
              clearNodeSelection(userInfo);
            } else if (toBeLinked.current != -1) {
              console.log("link node " + id + " to " + toBeLinked.current);

              // NOTE: since we used squeeze instead of trigger for adding nodes and links, we squeezeend, we will add link from nodecreator

              toBeLinked.current = -1;
            } else if (isMerge.current) {
              if (user) {
                // HACK: this is a hack to make sure the node is not picked by both hands
                if (
                  rightPickedObjects.length == 0 ||
                  leftPickedObjects.length == 0
                ) {
                ...
                mergeNodes(user, [rightNodeId, leftNodeId]);
              }
            } else {
              let nodeId = oid.slice(1);
    ...
    useXREvent("squeezestart", onSqeezeStart);

    useXREvent("squeezeend", onSqeezeEnd);

    const hand0 = useThree((state) => state.gl).xr.getHand(0);
    const hand1 = useThree((state) => state.gl).xr.getHand(1);

    const onSqeezeStartHand = useCallback(
      (e) => {
        console.log("hand pick", e.target, e.target.position);
        e.nativeEvent = { data: { handedness: e.target.handedness } };
        e.target.grip = e.target.grabArea;
        e.target.controller = e.target.grabArea;
        onSqeezeStart(e);
      },
      [rightPickedObjects, leftPickedObjects]
    );

    const onSqeezeEndHand = useCallback(
      (e) => {
        console.log("hand pick end", e);
        e.nativeEvent = { data: { handedness: e.target.handedness } };
        e.target.grip = e.target.grabArea;
        e.target.controller = e.target.grabArea;
        onSqeezeEnd(e);
    ...
    useEffect(() => {
      [hand0, hand1].forEach((hand) => {
        // hand.addEventListener("pinchstart", onSqeezeStartHand);
        // hand.addEventListener("pinchend", onSqeezeEndHand);
        hand.addEventListener("handgrab", onSqeezeStartHand);
        hand.addEventListener("handrelease", onSqeezeEndHand);
      });

      return () => {
        [hand0, hand1].forEach((hand) => {
          // hand.removeEventListener("pinchstart", onSqeezeStartHand);
          // hand.removeEventListener("pinchend", onSqeezeEndHand);
          hand.removeEventListener("handgrab", onSqeezeStartHand);
          hand.removeEventListener("handrelease", onSqeezeEndHand);
        });
      };
    }, [hand0, hand1, onSqeezeStartHand, onSqeezeEndHand]);
        ...
        <Interactive
          onSelect={(e) => {
            if (user && e.intersection?.instanceId)
              sendAction(user, e.intersection.instanceId, "graph");
          }}
        >
          <instancedMesh
            // castShadow
