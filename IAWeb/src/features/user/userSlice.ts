...
import { BoardcastMessage, UserInfo, UserInfo_ClientType } from "../../common/message";
...
interface UserState {
  ...
  userId: string | null;
  env: UserInfo_ClientType | null;
  boardcastMessage: BoardcastMessage | null;
  graphMode: GraphMode;
  graphControl: any | null;
  nodesNearbyCursor: { nodeId: string; weight: number }[];
}
...
export const userSlice = createSlice({
  ...
  reducers: {
    ...
    setUserEnv(state, action: PayloadAction<UserInfo_ClientType>) {
      state.env = action.payload;
    },
    setBoardcastMessage(
      state,
      action: PayloadAction<BoardcastMessage>
    ) {
    ...
    setNodesNearbyCursor(state, action: PayloadAction<{ nodeId: string; weight: number }[]>) {
