...
import { Position, UserInfo } from "../../common/message";

function setLine(line, start:Position, end:Position) {
  // console.log(start);
...
export default function UserFrustum({
  ...
  const color = showSelf ? "black" : getUserColor(user?.id);

  useFrame(() => {
    // let usersInRoom = store.getState().room.userList;
    // console.log("users in room: " + usersInRoom.length, usersInRoom.map(d => d.toObject()));

    // if (usersInRoom) {
    //   const user = usersInRoom[userIndex];
    if (user) {
      const frustum =
        store.getState().room.userFrustum[user.id + user.type];
      if (frustum) {
        let posN1 = frustum.n1!;
        let posN2 = frustum.n2!;
        let posN3 = frustum.n3!;
        let posN4 = frustum.n4!;

        let posF1 = frustum.f1!;
        let posF2 = frustum.f2!;
        let posF3 = frustum.f3!;
        let posF4 = frustum.f4!;
      ...
      let spatialInfo =
        store.getState().room.userSpatialInfo[user.id + user.type];
      if (spatialInfo) {
