...
import { getNodeById } from "./NodesInstance";
...
export default function NodeInstances({
  ...
  console.log("rendering static node instances");
  ...
  useFrame(() => {
    ...
    const clickedNodes = useGraphStore.getState().selectedNodeIds.map((id) => {
      return getNodeById(nodesRaw, id, nodeIdIndexMap);
    });
    ...
    if (instance) {
      ...
      otherHighlightedNodes.forEach((nodeStatus, i) => {
        const targetNodeId = getNodeIndex(`${nodeStatus.id}`, nodeIdIndexMap);
