...
// ];
const randomDocs2 = [
  "Week-of-Mon-20040503-4.txt_14",
  "Week-of-Mon-20040126-1.txt_14",
  "Week-of-Mon-20040112-4.txt_54",
  "Week-of-Mon-20031201-2.txt_44",
  "Week-of-Mon-20040621-4.txt_61",
  "Week-of-Mon-20040209-3.txt_1",
];
    ...
    Promise.all([
      ...
      let subdoc1 = docs2.filter((doc) => {
        return (
          ...
          randomDocs2.slice(0, 2).includes(doc.id)
        );
      });
      let subdoc2 = docs2.filter((doc) => {
        return (
          keyEvents2.slice(6, 12).includes(doc.id) ||
          randomDocs2.slice(2, 4).includes(doc.id)
        );
      });
      let subdoc3 = docs2.filter((doc) => {
        return (
          keyEvents2.slice(19).includes(doc.id) ||
          randomDocs2.slice(4, 6).includes(doc.id)
        );
      });
      console.log(
...
function cleanText(text: string) {
  ...
  .replace(/\r/g, "\n") // replace mac newline
  .replace(/ *\r?\n *([\w"'.,])/g, " $1") // replace newline at the start of a line
